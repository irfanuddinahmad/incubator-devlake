...
var CollectDailyUsageMeta = plugin.SubTaskMeta{
	...
	Description:      "Collect Cursor daily usage data per user from GET /teams/daily-usage-data",
	DomainTypes:      []string{plugin.DOMAIN_TYPE_CROSS},
}

// cursorDailyUsageResponse is the envelope returned by GET /teams/daily-usage-data.
...
// CollectDailyUsage fetches per-user daily usage metrics from the Cursor API
// with incremental sync support (starting from last successful run).
func CollectDailyUsage(taskCtx plugin.SubTaskContext) errors.Error {
	...
	err = collector.InitCollector(helper.ApiCollectorArgs{
		...
		ResponseParser: func(res *http.Response) ([]json.RawMessage, errors.Error) {
			...
			// The API may return a flat array or a paged envelope.
			...
			if jsonErr := json.Unmarshal(body, &rows); jsonErr != nil {
				return nil, errors.Default.Wrap(jsonErr, "failed to parse Cursor daily-usage response as array")
			}
			return rows, nil
		},
		Incremental: true,
