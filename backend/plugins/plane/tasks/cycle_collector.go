...
var CollectCyclesMeta = plugin.SubTaskMeta{
	...
	Description:      "Collect Plane cycles from the remote API",
...
func CollectCycles(taskCtx plugin.SubTaskContext) errors.Error {
	...
	collector, err := api.NewApiCollector(api.ApiCollectorArgs{
		RawDataSubTaskArgs: api.RawDataSubTaskArgs{
			...
			Table: RAW_CYCLE_TABLE,
		},
		ApiClient:   data.ApiClient,
		PageSize:    planeCyclePageSize,
		UrlTemplate: "api/v1/workspaces/{{ .Params.WorkspaceSlug }}/projects/{{ .Params.ProjectId }}/cycles/",
		Query: func(reqData *api.RequestData) (url.Values, errors.Error) {
			query := url.Values{}
			query.Set("per_page", strconv.Itoa(planeCyclePageSize))
			query.Set("cycle_view", "all")
