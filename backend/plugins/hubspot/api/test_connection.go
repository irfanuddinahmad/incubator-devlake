...
func testHubspotConnection(connection *models.HubspotConnection) errors.Error {
	apiClient, err := helper.NewApiClientFromConnection(context.Background(), basicRes, connection)
	if err != nil {
		return err
	}

	res, err := apiClient.Get("integrations/v1/me", nil, nil)
	if err != nil {
		return err
	}
...
// TestConnection validates a new HubSpot connection before it is saved.
func TestConnection(input *plugin.ApiResourceInput) (*plugin.ApiResourceOutput, errors.Error) {
	...
	if err := testHubspotConnection(connection); err != nil {
		return nil, plugin.WrapTestConnectionErrResp(basicRes, err)
	}
...
// TestExistingConnection validates a stored HubSpot connection with optional overrides.
